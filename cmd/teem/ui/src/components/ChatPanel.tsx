...
function ChatPanelInner({ teamID }: { teamID: string }) {
  ...
  // Tracks whether the user is currently pinned to the bottom. Driven
  // by a `scroll` event listener (below) rather than a layout effect,
  // because useLayoutEffect fires *after* DOM mutations: by the time
  // it ran, scrollHeight had already grown and the distance-from-
  // bottom math saw the post-commit state, not the operator's intent.
  // A scroll listener reflects the operator's actual scroll position
  // independent of render timing, and also captures the case where
  // they scroll up *during* a stream.
  ...
  // Watch the log's scroll position so wasAtBottomRef reflects the
  // operator's actual intent. Runs once on mount; the listener fires
  // synchronously on every user scroll *and* on any programmatic
  // scrollTop write below, so the ref stays accurate without us
  // having to peek at the DOM during render.
  useEffect(() => {
